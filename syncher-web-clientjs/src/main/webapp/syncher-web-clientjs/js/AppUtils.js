define([ 'jquery', 'jqueryBlockUi' ], function($) {
    var AppUtils = {
        ...
        pushEachInto : function(jsonArray, observableArray) {
            if (jsonArray) {
                _.each(jsonArray, function(jsonElement) {
                    observableArray.push(jsonElement);
                });
            }
        },
        applyConfigurationData : function(data, object) {
            object.casEnabled(data.casEnabled);
            object.username(data.username);
            object.syncherRunning(data.syncherRunning);
            AppUtils.pushEachInto(data.repositoriesToIgnore, object.repositoriesToIgnore);
            AppUtils.pushEachInto(data.groups, object.groups);
            object.schedule(data.schedule);
            
        },
        fetchConfiguration : function(successCallback, errorCallback, object) {
            $.ajax({
                type : "GET",
                url : '../api/syncher-web-restserver/configuration',
                dataType : 'json',
                success : function(portalData, status, response) {
                   AppUtils.applyConfigurationData(portalData, object)
                    if (successCallback) {
                        successCallback(portalData, status, response);
                    }

                },
                error : function() {
                    if (errorCallback) {
                        errorCallback(portalData, status, response);
                    }
                }
            });
        },
